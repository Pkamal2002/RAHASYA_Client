...
import { updatePassword } from '../store/slices/vaultSlice';
import CategoryManagerModal from './CategoryManagerModal';
import toast from 'react-hot-toast';

const EditPasswordModal = ({ isOpen, onClose, passwordData }) => {
  ...
  useEffect(() => {
    if (passwordData) {
      setFormData({
        ...
        department: passwordData.department || 'IT',
  ...
  }, [passwordData, isCategoryModalOpen]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    toast.promise(
      dispatch(updatePassword({ id: passwordData._id, data: formData })).unwrap(),
      {
        loading: 'Updating secret...',
        success: 'Secret updated successfully!',
        error: (err) => err.message || 'Failed to update secret',
      }
    );
    onClose();
  };
          ...
          >
            <div className="flex items-center justify-between mb-6 sm:mb-8">
              <div className="flex items-center space-x-3">
                ...
                <h2 className="text-xl sm:text-2xl font-bold text-white">Edit Secret</h2>
              </div>
